...
func TestTCPRouteReferenceGrant(t *testing.T) {
	...
	f := features.
		...
		WithSetup("configure TCP Deployments with Services and TCPRoutes", func(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			// otherNs, err := clusters.GenerateNamespace(context.Background(), cluster, helpers.LabelValueForTest(t))
			// assert.NoError(t, err)
			// t.Logf("created another namespace %s for test case %s", otherNs.Name, t.Name())
			// ctx = setInCtx(ctx, "tcproute_other_namespace", otherNs.Name)
			...
			deployment2 := generators.NewDeploymentForContainer(container2)
			deployment2, err = cluster.Client().AppsV1().Deployments(otherNs.Name).Create(ctx, deployment2, metav1.CreateOptions{})
			assert.NoError(t, err)
			cleaner.Add(deployment2)
			...
			service2, err = cluster.Client().CoreV1().Services(otherNs.Name).Create(ctx, service2, metav1.CreateOptions{})
			assert.NoError(t, err)
			cleaner.Add(service2)
			ctx = setInCtx(ctx, "service2", service2)
		...
		Assess("verifying that only the local tcpecho is responding without a ReferenceGrant", func(ctx context.Context, t *testing.T, _ *envconf.Config) context.Context {
			tcpGatewayURL := GetTCPURLFromCtx(ctx)
			// gatewayClient := GetFromCtxForT[*gatewayclient.Clientset](ctx, t)
			// tcpRoute := GetFromCtxForT[*gatewayapi.TCPRoute](ctx, t)
			// namespace := GetNamespaceForT(ctx, t)
			// otherNs := getInCtx[string, string](ctx, t, "otherNamespace")

			assertEventuallyResponseTCP(t, tcpGatewayURL, testUUID1)
			assert.Never(t, func() bool {
				return test.EchoResponds(test.ProtocolTCP, tcpGatewayURL, testUUID2) == nil
			}, consts.IngressWait, consts.WaitTick)
			return ctx
		}).
		Assess("verifying creating a ReferenceGrant that permits tcproute access", func(ctx context.Context, t *testing.T, _ *envconf.Config) context.Context {
			tcpGatewayURL := GetTCPURLFromCtx(ctx)
			gatewayClient := GetFromCtxForT[*gatewayclient.Clientset](ctx, t)
			tcproute := GetFromCtxForT[*gatewayapi.TCPRoute](ctx, t)
			namespace := GetNamespaceForT(ctx, t)
			otherNs := getInCtx[string](ctx, t, "tcproute_other_namespace")
			service2 := getInCtx[*corev1.Service](ctx, t, "service2")

			t.Logf("creating a ReferenceGrant that permits tcproute access from %s to services in %s", namespace, otherNs)
			...
			var err error
			grant, err = gatewayClient.GatewayV1beta1().ReferenceGrants(otherNs).Create(ctx, grant, metav1.CreateOptions{})
			assert.NoError(t, err)

			t.Log("verifying that requests reach both the local and remote namespace echo instances")
			assertEventuallyResponseTCP(t, tcpGatewayURL, testUUID1)
			assertEventuallyResponseTCP(t, tcpGatewayURL, testUUID2)
			...
			grant, err = gatewayClient.GatewayV1beta1().ReferenceGrants(otherNs).Update(ctx, grant, metav1.UpdateOptions{})
			assert.NoError(t, err)
			assertEventuallyResponseTCP(t, tcpGatewayURL, testUUID2)
			...
			_, err = gatewayClient.GatewayV1beta1().ReferenceGrants(otherNs).Update(ctx, grant, metav1.UpdateOptions{})
			assert.NoError(t, err)
